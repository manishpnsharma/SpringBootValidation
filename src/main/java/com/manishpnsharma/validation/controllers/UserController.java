...
public class UserController {
    ...
    public ResponseEntity<User> getUserById(@PathVariable Long id) {
        ...
        return userOptional.map(ResponseEntity::ok).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "NO User PRESENT WITH ID : " + id));
    }

    @GetMapping("/id/{id}")
    public ResponseEntity<?> getUserByIdExp(@PathVariable Long id) {
        Optional<User> userOptional = userService.getUserById(id);
        return userOptional.map(ResponseEntity::ok).orElseThrow(() -> new NoSuchUserExistsException(id, "NO USER PRESENT WITH ID : ", "HttpStatus"));
    }
    ...
    public ResponseEntity<User> userUpdate(@PathVariable Long id, @RequestBody User user) {
        User userUpdate = userService.getUserById(id).orElseThrow(() -> new NoSuchUserExistsException(id, "NO USER PRESENT WITH ID : ", "HttpStatus"));
